  ...
  context 'provide bank account details' do
    ...
    setup do
      ...
      should 'try to store bank account' do
        Billing::Gateway::Processor::Outcoming::Balanced.expects(:create_customer_with_bank_account!).with do |company|
      ...
      should 'handle inability to invalidate old account' do
        Billing::Gateway::Processor::Outcoming::Balanced.expects(:create_customer_with_bank_account!).raises(RuntimeError.new("Bank account should have been invalidated, but it's still valid for InstanceClient(id=1)"))
        refute @company.save
      ...
      context 'information missing' do
        ...
        should 'not store information if no bank account_number' do
          ...
          Billing::Gateway::Processor::Outcoming::Balanced.expects(:create_customer_with_bank_account!).never
          refute @company.save
