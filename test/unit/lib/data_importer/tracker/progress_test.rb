...
class DataImporter::Tracker::ProgressTest < ActiveSupport::TestCase

  setup do
    @data_upload = stub()
    @progress_tracker = DataImporter::Tracker::Progress.new(@data_upload, 1000)
  end

  context 'processing' do
    should 'track processing of location' do
      DataImporter::Tracker::Progress::Updater.any_instance.expects(:update).times(3)
    ...
    should 'not track processing of user' do
      DataImporter::Tracker::Progress::Updater.any_instance.expects(:update).never
