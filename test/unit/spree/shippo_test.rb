...
class ShippoTest < ActionView::TestCase
  ...
  def setup_stubs_for_shippo_order_testing(get_rates = true, clear_categories = false)
    shippo_mock_rates = [
    ...
    if get_rates
      ...
      shippo_api_mock.expects(:get_rates).times(1).with do |address_from_info, address_to_info, parcel_info, customs_item_info, customs_declaration_info|
  ...
  context 'shippo' do
    ...
    should 'create spree shipping methods if shippo enabled' do
      PlatformContext.current.instance.expects('shippo_enabled?').times(2).returns(true)
      
      order = setup_stubs_for_shippo_order_testing
    ...
    should 'not create spree shipping methods if shippo not enabled' do
      PlatformContext.current.instance.expects('shippo_enabled?').times(2).returns(false)

      order = setup_stubs_for_shippo_order_testing(false)
    ...
    should 'create parent shipping category if it does not exist' do
      ...
      order = setup_stubs_for_shippo_order_testing(true, true)

      order.line_items.map(&:variant).each { |v| v.reload }

      shipping_categories = order.line_items.map(&:variant).map(&:shipping_category).uniq
