...
class AnalyticWrapper::MixpanelApiTest < ActiveSupport::TestCase
  ...
  context '#initialize' do
    ...
    should 'store current instance id and current domain' do
      request_details_hash = { 'current_instance_id' => Instance.first.id, 'current_host' => 'www.example.com' }
  ...
  context '#track' do
    should 'trigger mixpanel track with distinct_id' do
      ...
      expected_properties = properties.merge(:distinct_id => wrapper.distinct_id, 'current_instance_id' => Instance.first.id)
      @mixpanel.expects(:track).with do |_event_name, _properties, _options|
    ...
    should 'apply session properties' do
      ...
      expected_properties = properties.merge(distinct_id: wrapper.distinct_id, source: 'google', current_instance_id: Instance.first.id).stringify_keys
      @mixpanel.expects(:track).with do |_event_name, _properties, _options|
    ...
    should 'apply request details' do
      ...
      expected_properties = properties.merge(distinct_id: wrapper.distinct_id, current_instance_id: Instance.first.id, current_host: 'www.example.com').stringify_keys
      @mixpanel.expects(:track).with do |_event_name, _properties, _options|
        _event_name == name && _properties.stringify_keys == expected_properties
      end
