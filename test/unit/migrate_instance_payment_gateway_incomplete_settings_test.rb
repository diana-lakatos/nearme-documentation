...
class MigrateInstancePaymentGatewaySettingsTest < ActiveSupport::TestCase
  ...
  context 'migrate settings' do
    ...
    should 'assert that settings where migrated correctly' do
      assert_not_equal @instance.instance_payment_gateways.get_settings_for(:stripe, :login, :test), "test_stripe_api_key"
      assert_not_equal @instance.instance_payment_gateways.get_settings_for(:stripe, :login, :live), "live_stripe_api_key"
      ...
      # remove all countries supported except US
      assert_equal @instance.country_instance_payment_gateways.where(instance_id: @instance.id).count, PaymentGateway.countries.count - ActiveMerchant::Billing::StripeGateway.supported_countries.count + 1
    end
  end
