...
class ReservationSmsNotifierTest < ActiveSupport::TestCase
  setup do
    ...
    @platform_context = PlatformContext.new.initialize_with_company(@reservation.company)
  end

  context '#notify_host_with_confirmation' do
    should "render with the reservation" do
      sms = ReservationSmsNotifier.notify_host_with_confirmation(@platform_context, @reservation)
  ...
  context '#notify_guest_with_state_change' do
    should "render with the reservation" do
      ...
      @reservation.confirm
      sms = ReservationSmsNotifier.notify_guest_with_state_change(@platform_context, @reservation)
