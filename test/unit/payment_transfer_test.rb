...
class PaymentTransferTest < ActiveSupport::TestCase
  ...
  context 'payout' do
    setup do
      Billing::Gateway::Processor::Outcoming::ProcessorFactory.stubs(:paypal_supported?).returns(true).once
      Billing::Gateway::Processor::Outcoming::ProcessorFactory.stubs(:receiver_supports_paypal?).returns(true).once
      @payment_transfer = @company.payment_transfers.build
      @payment_transfer.reservation_charges = @reservation_charges
    end

    should 'be not paid if attempt to payout failed' do
      Billing::Gateway::Outcoming.any_instance.expects(:payout).with { |hash| Money === hash[:amount] && @payment_transfer == hash[:reference] }.once.returns(stub(:success => false))
      @payment_transfer.save!
      refute @payment_transfer.transferred?
    end

    should 'be paid if attempt to payout succeeded' do
      Billing::Gateway::Outcoming.any_instance.expects(:payout).with { |hash| Money === hash[:amount] && @payment_transfer == hash[:reference] }.once.returns(stub(:success => true))
      @payment_transfer.save!
  ...
  context 'possible_automated_payout_not_supported?' do
    ...
    should "return true if possible processor exists but company has not provided settings" do
      Billing::Gateway::Outcoming.any_instance.stubs(:possible?).returns(false)
      Billing::Gateway::Outcoming.any_instance.stubs(:support_automated_payout?).returns(true)
    ...
    should "return false if there is no potential processor and company has not provided settings" do
      Billing::Gateway::Outcoming.any_instance.stubs(:possible?).returns(false)
      Billing::Gateway::Outcoming.any_instance.stubs(:support_automated_payout?).returns(false)
    ...
    should "return false if there is no possible processor and company has provided settings" do
      Billing::Gateway::Outcoming.any_instance.stubs(:possible?).returns(true)
      Billing::Gateway::Outcoming.any_instance.stubs(:support_automated_payout?).returns(false)
      refute @payment_transfer.possible_automated_payout_not_supported?
    end
