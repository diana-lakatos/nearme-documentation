...
class AfterSignupMailerTest < ActiveSupport::TestCase
  ...
  setup do
    ...
    @from = @platform_context.contact_email
  end
  ...
  context "version if user booked a listing" do
    ...
    should "use proper template" do
      mail = AfterSignupMailer.help_offer(@platform_context, @user)
      assert mail.html_part.body.include?("and congratulations on your first booked #{@platform_context.bookable_noun}!"), "and congratulations on your first booked #{@platform_context.bookable_noun}! was not found in\n#{mail.html_part.body}"
  ...
  test "version if user added a listing" do
    ...
    mail = AfterSignupMailer.help_offer(@platform_context, @user)
    assert mail.html_part.body.include?("Thanks for listing your #{@platform_context.bookable_noun} with Desks Near Me!")
    assert !mail.html_part.body.include?("The Desks Near Me Team")
  end

  test "version if user neither booked a listing nor added a listing" do
    mail = AfterSignupMailer.help_offer(@platform_context, @user)
    assert mail.html_part.body.include?("I saw that you signed up but haven't added a #{@platform_context.bookable_noun} for rent")
    assert !mail.html_part.body.include?("The Desks Near Me Team")
  end
