...
class ReservationMailerTest < ActiveSupport::TestCase
  ...
  test "#notify_guest_of_cancellation" do
    mail = ReservationMailer.notify_guest_of_cancellation(@request_context, @reservation)
    subject = "[#{@request_context.name}] A booking you made has been cancelled by the owner"
    ...
    assert_equal [@request_context.support_email], mail.bcc
  end

  test "#notify_guest_of_confirmation" do
    mail = ReservationMailer.notify_guest_of_confirmation(@request_context, @reservation)
  ...
  test "#notify_guest_of_expiration" do
    mail = ReservationMailer.notify_guest_of_expiration(@request_context, @reservation)
  ...
  test "#notify_guest_of_rejection" do
    mail = ReservationMailer.notify_guest_of_rejection(@request_context, @reservation)

    assert mail.html_part.body.include?(@reservation.listing.name)

    assert_equal [@reservation.owner.email], mail.to
    assert_equal "[#{@request_context.name}] A booking you made has been declined", mail.subject
    assert_equal [@request_context.support_email], mail.bcc
  end

  test "#notify_guest_with_confirmation" do
    mail = ReservationMailer.notify_guest_with_confirmation(@request_context, @reservation)
  ...
  test "#notify_host_of_cancellation" do
    mail = ReservationMailer.notify_host_of_cancellation(@request_context, @reservation)

    assert mail.html_part.body.include?(@reservation.listing.creator.name)
    assert mail.html_part.body.include?(@expected_dates)

    assert_equal [@reservation.listing.creator.email], mail.to
    assert_equal [@request_context.support_email, @reservation.listing.location.email], mail.bcc
  end

  test "#notify_host_of_confirmation" do
    mail = ReservationMailer.notify_host_of_confirmation(@request_context, @reservation)
  ...
  test "#notify_host_of_expiration" do
    mail = ReservationMailer.notify_host_of_expiration(@request_context, @reservation)
  ...
  test "#notify_host_with_confirmation" do
    mail = ReservationMailer.notify_host_with_confirmation(@request_context, @reservation)
  ...
  test "#notify_host_without_confirmation" do
    mail = ReservationMailer.notify_host_without_confirmation(@request_context, @reservation)
  ...
  test "send to contact person if exists" do
    ...
    ['notify_host_of_cancellation', 'notify_host_of_confirmation', 'notify_host_of_expiration',
     'notify_host_with_confirmation', 'notify_host_without_confirmation'].each do |method|
      mail = ReservationMailer.send(method, @request_context, @reservation)
