...
class InstanceAdmin::SettingsControllerTest < ActionController::TestCase
  ...
  context 'authorization' do
    ...
    should 'end with success if user is authorized to view settings' do
      ...
      InstanceAdminAuthorizer.any_instance.stubs(:authorized?).with(InstanceAdmin::SettingsController).returns(true)
      get :show
    ...
    should 'redirect user to instance admin path if he is authorized for analytics' do
      InstanceAdminAuthorizer.any_instance.stubs(:authorized?).with(InstanceAdmin::AnalyticsController).returns(true)
      InstanceAdminAuthorizer.any_instance.stubs(:authorized?).with(InstanceAdmin::SettingsController).returns(false)
      get :show
