Feature: Emails should be sent out informing parties about reservations
  ...
  Scenario: reservation confirmations required (no comment)
    ...
    And the 1st email should contain "Keith Contractor has made a reservation for Mocra on October 15, 2010"
    ...
    And the 2nd email should contain "You have made a reservation for Mocra on October 15, 2010."

  Scenario: reservation confirmations required (with comment)
    Given a listing: "Mocra" exists with name: "Mocra", creator: user "Bo Jeanes", confirm_reservations: true
    And I am logged in as user "Keith Contractor"
    When I go to the listing's page
    And I follow the reservation link for "15th October 2010"
