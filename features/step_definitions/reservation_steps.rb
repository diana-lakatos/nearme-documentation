Given /^(.*) has a( |n un)confirmed reservation for (.*)$/ do |lister, confirmed, reserver|
  ...
  reservation = @listing.reserve!(Controller::RequestContext.new, reserver, [next_regularly_available_day], 1)
  unless confirmed != " "
    reservation.confirm!
    reservation.save
  end
...
When /^(.*) books a space for that listing$/ do |person|
  listing.reserve!(Controller::RequestContext.new, User.find_by_name(person), [next_regularly_available_day], 1)
end
