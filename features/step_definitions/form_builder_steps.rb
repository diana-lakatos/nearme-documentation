...
When(/^I update my profile and trigger all validation errors$/) do
  page.find('a[data-association="everything_model"]').click
  click_button 'Save'
end
...
Then(/^All error messages for the form are correctly displayed$/) do
  ...
  ['.user_avatar',
   '.user_profiles_buyer_properties_textarea_field',
   '.user_profiles_buyer_properties_radio_button_fields',
   '.user_profiles_buyer_properties_string_field',
  ...
  within('.user_profiles_buyer_categories_Industry') do
...
When(/^I add two and remove one customization and submit form again to see what happens$/) do
  ...
  within('.nested-fields-set > .nested-fields') do
    attach_file find('input[type="file"].attachment', visible: false)[:name], File.join(Rails.root, 'test', 'assets', 'bully.jpeg')
    attach_file find('input[type="file"].image', visible: false)[:name], File.join(Rails.root, 'test', 'assets', 'foobear.jpeg')
    find('a.remove_fields').click
  end

  page.find('a[data-association="everything_model"]').click
  within('.nested-fields-set > .nested-fields') do
    attach_file find('input[type="file"].attachment', visible: false)[:name], File.join(Rails.root, 'test', 'assets', 'foobear.jpeg')
    attach_file find('input[type="file"].image', visible: false)[:name], File.join(Rails.root, 'test', 'assets', 'bully.jpeg')
    fill_in find('input.string')[:name], with: 'my name'
  end
  click_button 'Save'
end
...
Then(/^The first set is removed and other is swappped$/) do
  ...
  within('.nested-container') do
    ...
    assert_equal 'my name', find('.user_profiles_buyer_customizations_everything_model_properties_name input').value
  end
...
When(/^I fill rest of the form$/) do
  ...
  fill_in 'user[profiles_attributes][buyer_attributes][properties_attributes][textarea_field]', with: 'text area value'
  within('.user_profiles_buyer_properties_radio_button_fields') do
    page.execute_script("document.getElementById('user_profiles_attributes_buyer_attributes_properties_attributes_radio_button_fields_two').checked = true")
  end
  fill_in 'user[profiles_attributes][buyer_attributes][properties_attributes][string_field]', with: 'string value'
  fill_in 'user[profiles_attributes][buyer_attributes][properties_attributes][decimal_field]', with: 35.23
  within('.user_profiles_buyer_properties_array_checkboxes') do
    page.execute_script("document.getElementById('user_profiles_attributes_buyer_attributes_properties_attributes_array_checkboxes_one').checked = true")
    page.execute_script("document.getElementById('user_profiles_attributes_buyer_attributes_properties_attributes_array_checkboxes_three').checked = true")
  end
  page.execute_script("document.getElementById('user_profiles_attributes_buyer_attributes_properties_attributes_boolean_checkbox').checked = true")
...
Then(/^all data is properly stored in DB and the form is re\-rendered with those value filled$/) do
  ...
  assert_equal 'text area value', find('#user_profiles_attributes_buyer_attributes_properties_attributes_textarea_field').value

  assert find('#user_profiles_attributes_buyer_attributes_properties_attributes_radio_button_fields_two').checked?
  assert_equal 'string value', find('#user_profiles_attributes_buyer_attributes_properties_attributes_string_field').value
  assert_equal '35.23', find('#user_profiles_attributes_buyer_attributes_properties_attributes_decimal_field').value

  assert find('#user_profiles_attributes_buyer_attributes_properties_attributes_array_checkboxes_one').checked?
  refute find('#user_profiles_attributes_buyer_attributes_properties_attributes_array_checkboxes_two').checked?
  assert find('#user_profiles_attributes_buyer_attributes_properties_attributes_array_checkboxes_three').checked?

  assert find('#user_profiles_attributes_buyer_attributes_properties_attributes_boolean_checkbox').checked?

  within('.user_profiles_buyer_categories_Area') do
  ...
  within('.user_profiles_buyer_categories_Languages') do
