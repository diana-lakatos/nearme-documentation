...
Then /^unaccepted order item should be generated$/ do
  ...
  assert_equal 1100000, invoice.total_amount_cents
  assert_equal 1100000, invoice.total_amount_cents
end
...
And /^I stub sending stripe documents$/ do
  ...
  MerchantAccount::StripeConnectMerchantAccount.any_instance.stubs(:tos_acceptance_timestamp).returns(1487963976)
end
...
Given /^credit_card is prcessed with Stripe$/ do
  ...
  offer.payment_subscription.credit_card_attributes = FactoryGirl.attributes_for(:credit_card_attributes).reject {|k,v| k == :response }
