# coding: utf-8
Given /^a( disabled)?( indexed)? listing( with nil prices)? in (.*) exists?$/ do |disabled, indexed, nil_prices, city|
...
Given /^(.*) does( not)? require confirmation for his listing$/ do |person, with_confirmation|
  ...
  @listing.save
  @listing.company.update_attribute(:creator_id, User.find_by(name: person).id)
  @listing.reload
end
...
Then /^I (do not )?see a search result for the ([^\$].*) listing$/ do |negative, city|
  listing = instance_variable_get("@listing_in_#{city.downcase.tr(' ', '_')}")
  if negative
