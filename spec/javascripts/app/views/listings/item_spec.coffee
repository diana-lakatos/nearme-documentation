define "views/listings/item_spec", ["models/listing", "views/listings/item"], (ListingModel, ListingView) ->
  describe "Listing view", ->
    ...
    it "should have availability_rules_defer == 1 when trigger for defer availability is triggered", ->
      $deferAvailabilityTrigger = $(@listingView._availabilityTrigger, $hook).filter("[id=availability_rules_defer]")
      spyEvent = spyOnEvent($deferAvailabilityTrigger, 'click')
      $deferAvailabilityTrigger.click()
      expect(spyEvent).toHaveBeenTriggered()
      expect($('#defer_availability_rules', $hook).val()).toEqual '1'
