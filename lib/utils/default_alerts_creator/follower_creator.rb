class Utils::DefaultAlertsCreator::FollowerCreator < Utils::DefaultAlertsCreator::WorkflowCreator
  ...
  def create_user_followed_transactable!
    create_alert!({associated_class: WorkflowStep::FollowerWorkflow::UserFollowedTransactable, name: 'user_followed_transactable', path: 'user_mailer/user_followed_transactable', subject: "{{ user.name }} started following {{ transactable.name }}", alert_type: 'email', recipient_type: nil, bcc_type: 'collaborators' })
  end
