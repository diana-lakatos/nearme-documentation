class Utils::DefaultAlertsCreator::OfferCreator < Utils::DefaultAlertsCreator::WorkflowCreator
  ...
  def notify_guest_of_expiration_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::Expired, name: 'notify_guest_of_expiration', path: 'offer_mailer/notify_guest_of_expiration', subject: "[{{platform_context.name}}] Your booking for '{{reservation.transactable.name}}' at {{reservation.location.street}} has expired", alert_type: 'email', recipient_type: 'enquirer'})
  end
  ...
  def notify_host_of_cancellation_by_guest_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::EnquirerCancelled, name: 'Notify host of guest cancellation', path: 'offer_mailer/notify_host_of_cancellation_by_guest', subject: "[{{platform_context.name}}] {{reservation.owner.first_name }} cancelled a booking for '{{reservation.transactable.name}}' at {{reservation.location.street}}", alert_type: 'email', recipient_type: 'lister'})
  end

  def notify_guest_of_cancellation_by_host_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::HostCancelled, name: 'Notify guest of host cancellation', path: 'offer_mailer/notify_guest_of_cancellation_by_host', subject: "[{{platform_context.name}}] Your booking for '{{reservation.transactable.name}}' at {{reservation.location.street}} was cancelled by the host", alert_type: 'email', recipient_type: 'enquirer'})
  end

  def notify_host_of_cancellation_by_host_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::HostCancelled, name: 'Notify host of host cancellation', path: 'offer_mailer/notify_host_of_cancellation_by_host', subject: "[{{platform_context.name}}] You just declined a booking", alert_type: 'email', recipient_type: 'lister'})
  end

  def notify_guest_reservation_created_and_confirmed_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::CreatedWithAutoConfirmation, name: 'Notify guest of confirmation', path: 'offer_mailer/notify_guest_of_confirmation', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}}, your booking has been confirmed", alert_type: 'email', recipient_type: 'enquirer', custom_options: {'booking_calendar_attachment_name' => 'booking.ics'}})
  end

  def notify_host_reservation_created_and_confirmed_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::CreatedWithAutoConfirmation, name: 'notify_host_without_confirmation', path: 'offer_mailer/notify_host_without_confirmation', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}} just booked your {{listing.transactable_type.bookable_noun}}!",  alert_type: 'email', recipient_type: 'lister'})
  end

  def notify_host_reservation_created_and_pending_confirmation_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::CreatedWithoutAutoConfirmation, name: 'Ask host for confirmation email', path: 'offer_mailer/notify_host_with_confirmation', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}} just booked your {{listing.transactable_type.bookable_noun}}!", alert_type: 'email', recipient_type: 'lister'})
  end

  def notify_host_reservation_created_and_pending_confirmation_sms!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::CreatedWithoutAutoConfirmation, name: 'Ask host for confirmation sms', path: 'reservation_sms_notifier/notify_host_with_confirmation', alert_type: 'sms', recipient_type: 'lister'})
  end

  def notify_guest_reservation_created_and_pending_confirmation_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::CreatedWithoutAutoConfirmation, name: 'Inform guest about pending confirmation', path: 'offer_mailer/notify_guest_with_confirmation', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}}, your booking is pending confirmation", alert_type: 'email', recipient_type: 'enquirer'})
  end

  def notify_guest_reservation_confirmed_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::ManuallyConfirmed, name: 'Notify guest of confirmation', path: 'offer_mailer/notify_guest_of_confirmation', alert_type: 'email', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}}, your booking has been confirmed", recipient_type: 'enquirer', custom_options: {'booking_calendar_attachment_name' => 'booking.ics'}})
  end
  ...
  def notify_guest_reservation_confirmed_sms!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::ManuallyConfirmed, name: 'notify_guest_of_confirmation_sms', path: 'reservation_sms_notifier/notify_guest_with_state_change', alert_type: 'sms', recipient_type: 'enquirer'})
  end

  def notify_guest_reservation_host_cancel_sms!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::HostCancelled, name: 'notify_guest_of_confirmation_sms', path: 'reservation_sms_notifier/notify_guest_with_state_change', alert_type: 'sms', recipient_type: 'enquirer'})
  end

  def notify_guest_reservation_reject_sms!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::Rejected, name: 'notify_guest_of_confirmation_sms', path: 'reservation_sms_notifier/notify_guest_with_state_change', alert_type: 'sms', recipient_type: 'enquirer'})
  end
  def notify_guest_reservation_rejected_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::Rejected, name: 'notify_guest_of_rejection', path: 'offer_mailer/notify_guest_of_rejection', subject: "[{{platform_context.name}}] Can we help, {{reservation.owner.first_name}}?", alert_type: 'email', recipient_type: 'enquirer'})
  end

  def notify_guest_of_payment_request_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::PaymentRequest, name: 'notify_guest_of_payment_request_email', path: 'offer_mailer/notify_guest_of_payment_request', subject: "[{{platform_context.name}}] Your booking for '{{reservation.transactable.name}}' at {{reservation.location.street}} requires payment", alert_type: 'email', recipient_type: 'enquirer'})
  end
  ...
  def request_rating_of_guest_from_host_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::EnquirerRatingRequested, name: 'request_rating_of_guest_from_host', path: 'rating_mailer/request_rating_of_guest_from_host', subject: "[{{platform_context.name}}] How was your experience hosting {{@reservation.owner.first_name}}?", alert_type: 'email', recipient_type: 'lister'})
  end

  def request_rating_of_host_from_guest_email!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::HostRatingRequested, name: 'request_rating_of_host_from_guest', path: 'rating_mailer/request_rating_of_host_from_guest', subject: "[{{platform_context.name}}] How was your experience at '{{reservation.transactable.name}}'?", alert_type: 'email', recipient_type: 'enquirer'})
  end
  ...
  def notify_host_of_approved_payment!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::EnquirerApprovedPayment, name: 'notify_host_of_approved_payment', path: 'offer_mailer/notify_host_of_approved_payment', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}} confirmed payment!",  alert_type: 'email', recipient_type: 'lister'})
  end
  ...
  def notify_host_of_declined_payment!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::EnquirerDeclinedPayment, name: 'notify_host_of_declined_payment', path: 'offer_mailer/notify_host_of_declined_payment', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}} declined payment!",  alert_type: 'email', recipient_type: 'lister'})
  end

  def notify_guest_of_submitted_checkout!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::HostSubmittedCheckout, name: 'notify_guest_of_submitted_checkout', path: 'offer_mailer/notify_guest_of_submitted_checkout', subject: "[{{platform_context.name}}] {{reservation.transactable.name}} submitted invoice",  alert_type: 'email', recipient_type: 'enquirer'})
  end

  def notify_guest_of_submitted_checkout_with_failed_authorization!
    create_alert!({associated_class: WorkflowStep::OfferWorkflow::HostSubmittedCheckoutButAuthorizationFailed, name: 'notify_guest_of_submitted_checkout_with_failed_authorization', path: 'offer_mailer/notify_guest_of_submitted_checkout_with_failed_authorization', subject: "[{{platform_context.name}}] {{reservation.transactable.name}} submitted invoice",  alert_type: 'email', recipient_type: 'enquirer'})
  end
