class Utils::DefaultAlertsCreator::OrderItemCreator < Utils::DefaultAlertsCreator::WorkflowCreator
  ...
  def create_notify_enquirer_rejected_order_item!
    create_alert!({associated_class: WorkflowStep::OrderItemWorkflow::Rejected, name: 'notify_enquirer_rejected_order_item', path: 'order_item_mailer/notify_enquirer_rejected_order_item', subject: '[{{platform_context.name}}] {{lister.first_name}} has rejected invoice!', alert_type: 'email', recipient_type: 'lister'})
  end
