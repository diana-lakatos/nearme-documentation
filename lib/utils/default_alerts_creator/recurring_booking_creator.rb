class Utils::DefaultAlertsCreator::RecurringBookingCreator < Utils::DefaultAlertsCreator::WorkflowCreator
  ...
  def notify_guest_of_expiration_email!
    create_alert!({associated_class: WorkflowStep::RecurringBookingWorkflow::Expired, name: 'notify_guest_of_expiration', path: 'recurring_booking_mailer/notify_guest_of_expiration', subject: "[{{platform_context.name}}] Your recurring booking for '{{reservation.listing.name}}' at {{reservation.location.street}} has expired", alert_type: 'email', recipient_type: 'enquirer'})
  end
  ...
  def notify_host_of_cancellation_by_guest_email!
    create_alert!({associated_class: WorkflowStep::RecurringBookingWorkflow::GuestCancelled, name: 'Notify host of guest cancellation', path: 'recurring_booking_mailer/notify_host_of_cancellation_by_guest', subject: "[{{platform_context.name}}] {{reservation.owner.first_name }} cancelled a recurring booking for '{{reservation.listing.name}}' at {{reservation.location.street}}", alert_type: 'email', recipient_type: 'lister'})
  end

  def notify_guest_of_cancellation_by_host_email!
    create_alert!({associated_class: WorkflowStep::RecurringBookingWorkflow::HostCancelled, name: 'Notify guest of host cancellation', path: 'recurring_booking_mailer/notify_guest_of_cancellation_by_host', subject: "[{{platform_context.name}}] Your recurring booking for '{{reservation.listing.name}}' at {{reservation.location.street}} was cancelled by the host", alert_type: 'email', recipient_type: 'enquirer'})
  end
  ...
  def notify_guest_recurring_booking_created_and_confirmed_email!
    create_alert!({associated_class: WorkflowStep::RecurringBookingWorkflow::CreatedWithAutoConfirmation, name: 'Notify guest of confirmation', path: 'recurring_booking_mailer/notify_guest_of_confirmation', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}}, your recurring booking has been confirmed", alert_type: 'email', recipient_type: 'enquirer'})
  end

  def notify_host_recurring_booking_created_and_confirmed_email!
    create_alert!({associated_class: WorkflowStep::RecurringBookingWorkflow::CreatedWithAutoConfirmation, name: 'notify_host_without_confirmation', path: 'recurring_booking_mailer/notify_host_without_confirmation', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}} just booked your {{platform_context.bookable_noun}}!",  alert_type: 'email', recipient_type: 'lister'})
  end
  ...
  def notify_guest_recurring_booking_created_and_pending_confirmation_email!
    create_alert!({associated_class: WorkflowStep::RecurringBookingWorkflow::CreatedWithoutAutoConfirmation, name: 'Inform guest about pending confirmation', path: 'recurring_booking_mailer/notify_guest_with_confirmation', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}}, your recurring booking is pending confirmation", alert_type: 'email', recipient_type: 'enquirer'})
  end

  def notify_guest_recurring_booking_confirmed_email!
    create_alert!({associated_class: WorkflowStep::RecurringBookingWorkflow::ManuallyConfirmed, name: 'Notify guest of confirmation', path: 'recurring_booking_mailer/notify_guest_of_confirmation', alert_type: 'email', subject: "[{{platform_context.name}}] {{reservation.owner.first_name}}, your recurring booking has been confirmed", recipient_type: 'enquirer'})
  end
  ...
  def notify_guest_recurring_booking_rejected_email!
    create_alert!({associated_class: WorkflowStep::RecurringBookingWorkflow::Rejected, name: 'notify_guest_of_rejection', path: 'recurring_booking_mailer/notify_guest_of_rejection', subject: "[{{platform_context.name}}] Can we help, {{reservation.owner.first_name}}?", alert_type: 'email', recipient_type: 'enquirer'})
  end
