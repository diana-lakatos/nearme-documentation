module Responders
  ...
  protected
    ...
    def set_flash_message!
      if has_errors?
        status = Responders::ApiFlashResponder.flash_keys.last
        set_flash(status, @alert)
      else
        status = Responders::ApiFlashResponder.flash_keys.first
      ...
      message = Responders::ApiFlashResponder.helper.t options[:default].shift, options
    ...
    def flash_defaults_by_namespace(status) #:nodoc:
      defaults = []
      slices   = controller.controller_path.split('/').delete_if { |el| el =~ /\Av\d+\z/ }
      lookup   = Responders::ApiFlashResponder.namespace_lookup

      begin
        if controller.send(:form_configuration).present?
          form_configuration_scope = :"flash.#{slices.fill(controller.controller_name, -1).join('.')}.#{controller.send(:form_configuration).name}.#{status}"
          defaults << :"#{form_configuration_scope}_html"
          defaults << form_configuration_scope
        end
