namespace :volte do
  ...
  class VolteSetup
    ...
    def expire_cache
      CacheExpiration.send_expire_command 'RebuildInstanceView', instance_id: 194
    #   CacheExpiration.send_expire_command 'Translation', instance_id: 194
      CacheExpiration.send_expire_command 'RebuildCustomAttributes', instance_id: 194
      Rails.cache.clear
    end
    ...
    def read_template(name)
      File.read(File.join(Rails.root, 'lib', 'tasks', 'volte', 'templates', name))
    end

    def get_page_content(filename)
      File.read(File.join(Rails.root, 'lib', 'tasks', 'volte', 'pages', filename))
    end

    # def create_email(path)
    #   body = File.read(File.join(Rails.root, 'lib', 'tasks', 'volte', 'templates', 'mailers', path + '.html.liquid'))
