...
namespace :litvault do
  ...
  task setup: :environment do
    time = Benchmark.realtime do
      ...
      InstanceProfileType.find(580).update_columns(        onboarding: true,
...
class LitvaultSetup
  ...
  def create_payment_gateway!
    pg = PaymentGateway::StripePaymentGateway.first_or_initialize(        instance_id: 198,
                                                                          test_settings: {
  ...
  # search for YAML front matter
    ...
    def create_custom_attribute(object, name, hash  )
      hash = hash.with_indifferent_acces  s
      attr = object.custom_attributes.where(            name: name).first_or_initializ  e
    ...
    children.each do |child  |
      name = (child.is_a? Hash) ? child['name'] : chil  d
    ...
    if attributes.size ==   0
      unused_attrs = object.custom_attribute  s
    els  e
    ...
    def default_category_propertie  s
      {
      mandatory: false  ,
    ...
    def default_attribute_propertie  s
      {
