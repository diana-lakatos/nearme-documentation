class PostActionMailerPreview < MailView
  ...
  def instance_created
    instance_admin = (InstanceAdmin.joins({:user => :listings}).first || InstanceAdmin.create(:instance_id => Instance.default_instance.id, :user_id => Transactablefirst.creator.id))
