...
class AnalyticWrapper::MixpanelApi
  ...
  def track_charge(amount, user_id = nil)
    user_id ||= distinct_id
    MixpanelApiJob.perform(@mixpanel, :track_charge, user_id, amount)
    Rails.logger.info "Tracked charge: user #{user_id}, amount: #{amount}$"
  end
