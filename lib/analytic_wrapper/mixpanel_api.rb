...
class AnalyticWrapper::MixpanelApi
  ...
  def apply_user(user, options = { :alias => false })
    ...
    if options[:alias] && anonymous_identity
      MixpanelApiJob.perform(@mixpanel, :alias, distinct_id, { :distinct_id => anonymous_identity }) if DesksnearMe::Application.config.perform_mixpanel_requests
  ...
  def track(event_name, properties, options = {})
    ...
    MixpanelApiJob.perform(@mixpanel, :track, event_name, properties, options) if DesksnearMe::Application.config.perform_mixpanel_requests
  ...
  def set_person_properties(properties)
    MixpanelApiJob.perform(@mixpanel, :set, distinct_id, properties) if DesksnearMe::Application.config.perform_mixpanel_requests
  ...
  def charge(amout, time = nil, options = {})
    MixpanelApiJob.perform(@mixpanel, :track_charge, distinct_id, amount, time, options) if DesksnearMe::Application.config.perform_mixpanel_requests
  end
  ...
  def track_charge_against_user(user, amount, time = nil, options = {})
    MixpanelApiJob.perform(@mixpanel, :track_charge, user.id, amount, time, options) if DesksnearMe::Application.config.perform_mixpanel_requests
  end
