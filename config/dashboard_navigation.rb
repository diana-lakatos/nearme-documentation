SimpleNavigation::Configuration.run do |navigation|
  ...
  def dashboard_nav_item(nav, key = nil, path = nil, options = {})
    ...
    nav.item key_controller, options[:link_text], path, unless: lambda { options[:unless] }, if: lambda { options[:if] }, highlights_on: options[:highlights_on]
  end

  navigation.id_generator = Proc.new {|key| "dashboard-nav-#{key}"}
