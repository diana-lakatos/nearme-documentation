...
class ShippoPurchaseRateJob < Job
  ...
  def perform
    ...
    if purchase_result.present?
      @shipping_method.order.update_attribute(:shippo_rate_purchased_at, Time.now)
      @shipping_method.update_attribute(:shippo_label_url, purchase_result.label_url)
      @shipping_method.update_attribute(:shippo_tracking_number, purchase_result.tracking_number)
    else
