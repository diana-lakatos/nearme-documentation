class ApplicationController < ActionController::Base
  ...
  def ensure_system_shipping_categories_copied
    if @company.blank?
      existing_shipping_categories_ids = Spree::ShippingCategory.not_system_profiles.where(user_id: current_user.id).collect(&:from_system_shipping_category_id)
    else
    ...
    system_shipping_categories_to_be_created.each do |shipping_category_to_copy|
      ...
      shipping_category_to_copy.shipping_methods.each do |shipping_method_to_copy|
        ...
        shipping_method.user_id = current_user.id
        shipping_method.calculator = shipping_method_to_copy.calculator
