class Manage::Listings::ReservationsController < ApplicationController
  ...
  def confirm
    if @reservation.confirm
      ...
      ReservationMailer.enqueue.notify_host_of_confirmation(@reservation.id)
  ...
  def reject
    if @reservation.reject(rejection_reason)
      ...
      ReservationMailer.enqueue.notify_guest_of_rejection(@reservation.id)
  ...
  def host_cancel
    if @reservation.host_cancel
      ReservationMailer.enqueue.notify_guest_of_cancellation(@reservation.id)
