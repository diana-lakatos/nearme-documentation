class Manage::Listings::ReservationsController < ApplicationController
  ...
  def confirm
    if @reservation.confirm
      ...
      MailerJob.perform(ReservationMailer, :notify_host_of_confirmation, @reservation)
  ...
  def reject
    if @reservation.reject(rejection_reason)
      ...
      MailerJob.perform(ReservationMailer, :notify_guest_of_rejection, @reservation)
  ...
  def host_cancel
    if @reservation.host_cancel
      MailerJob.perform(ReservationMailer, :notify_guest_of_cancellation, @reservation)
