    ...
    def create
      if @reservation_request.process
        if @reservation_request.confirm_reservations?
          ...
          reservation.schedule_expiry(request_context)
          ReservationMailer.enqueue.notify_host_with_confirmation(request_context, reservation)
          ReservationMailer.enqueue.notify_guest_with_confirmation(request_context, reservation)
          begin
            ...
            if e.message.include?('is not a valid phone number')
              reservation.host.notify_about_wrong_phone_number(request_context)
            else
          ...
          ReservationMailer.enqueue.notify_host_without_confirmation(request_context, reservation)
