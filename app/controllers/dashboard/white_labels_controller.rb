class Dashboard::WhiteLabelsController < Dashboard::BaseController
  ...
  def upload_image
    ...
    if @theme.save(validate: false)
      render :text => {
        :url => @theme.send("#{@image_param}_url"),
        :id => @image_param,
        :resize_url =>  edit_theme_image_instance_admin_theme_design_path(@image_param),
        :thumbnail_dimensions => {},
        :destroy_url => destroy_theme_image_instance_admin_theme_design_path(@image_param)
  ...
  def edit_image
    if request.xhr?
      render partial: 'instance_admin/theme/design/resize_form',
        :locals => { :form_url => update_theme_image_instance_admin_theme_design_path(@image_param),
                     :object => @image,
                     :object_url => @theme.send("#{@image_param}_url", :original) }
    end
  end
  ...
  def find_company
    @company = params[:id].present? ? current_user.companies.find(params[:id]) : current_user.companies.first
  end
