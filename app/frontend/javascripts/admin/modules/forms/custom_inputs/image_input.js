...
class ImageInput {
  ...
  _initializeFileUpload(){
    ...
    // this._ui.input.fileupload({
    //   url: this._ui.input.dataset.uploadUrl,
    //   paramName: this._ui.input.dataset.uploadName,
    //   dataType: 'json',
    //   dropZone: this._ui.dropZone,
    //   formData: (form)=>{
    //     const = params = form.clone();
    //     params.querySelector("input[name=_method]").remove();
    //     params.serializeArray()
    //     return params;
    //   }

    //   start: ()=>
    //     this._ui.fieldWrapper.classList.add('progress--active');

    //   stop: =>
    //     this._ui.field@fieldWrapper.removeClass('progress--active')
  ...
  _rebindEvents(){
        // this._ui.collection.querySelector('[data-action-preview]').swipebox()
  }
  ...
  _createCollectionItem(data){
    ...
    if (this._hasCaptions){
      container.insertAdjacentHTML('beforend',`<span class='caption'><input type='text' name='${this._objectName}[${this._modelName}s_attributes][${data.id}][caption]' value='' placeholder='${this._captionPlaceholder}'></span>`);
    }
