...
const defaults = { wrapperClass: '.form-group, .control-group' };
...
class ValidatableField {
  ...
  constructor(
    ...
    this.setup(validations).then(validators => {
  ...
  process(): Promise<mixed> {
    ...
    return new Promise(resolve => {
      Promise
        .all(
          this.validators.map(
            (validator: Validator): Promise<mixed> => validator.process(this.field.value)
          )
        )
        .then(() => {
          resolve();
  ...
  addError(message: string) {
    this.wrapper.classList.add('error', 'has-error');
    // TODO: When deprecating, use one class only
    let err = document.createElement('span');
    err.classList.add('help-block', 'error-block');
    // TODO: When deprecating, use single class only
  ...
  removeError() {
    this.wrapper.classList.remove('error', 'has-error');
    // TODO: When deprecating, use one class only
    let errorMessage = this.wrapper.querySelector('span.help-block');
    // TODO: This should be refactored and configurable
