...
PhotoView = function() {
  ...
  PhotoView.prototype.singlePhotoHtml = function() {
    ...
    cropLink = $(
      '<button type="button" class="button-a small action--edit" data-resize-photo data-modal title="Rotate & Crop"><span class="intelicon-edit"></span></button>'
    );
    cropLink.attr('data-href', this.data.resize_url);
    cropLink.attr('data-id', this.data.id);
    actions.append(cropLink);
    deleteLink = $(
      '<button class="button-a small red action--remove" type="button" data-delete-photo title="Delete photo"><span class="intelicon-trash-outlined"></span></button>'
    );
