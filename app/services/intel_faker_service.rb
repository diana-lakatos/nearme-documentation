class IntelFakerService
  ...
  def self.feed_items(number = 5)
    (1..number).inject([]) { |result, n| result << OpenStruct.new(display_name: name, description: feed_item_description, date: "#{Random.rand(1..60)} min ago", avatar_path: "community/silhouette_#{Random.rand(1..7)}.png") }.sort {| a, b | a[:date].to_i <=> b[:date].to_i }
  end

  def self.projects(number = 4)
    (1..number).inject([]) { |result, n| result << OpenStruct.new(title: project_title, date: 'April 18th, 2015', description: DESCRIPTION, collaborators: Random.rand(5..15), followers: Random.rand(50..500), comments: Random.rand(25..180)) }
  end

  def self.people(number = 10)
    (1..number).inject([]) { |result, n| result << OpenStruct.new(name: name, title: job_title) }
  end
  ...
  def self.job_title
    JOB_TITLES.shuffle.first
  end

  def self.project_title
    PROJECT_TITLES.shuffle.first
  end

  def self.feed_item_description
    FEED_ITEM_DESCRIPTION.shuffle.first
  end
