      ...
      Class.new(self) do
        configuration.each do |transactable_type_name, fields|
          ...
          collection :"#{transactable_type_name}",
                     ...
                     populator: POPULATOR,
                     prepopulator: ->(*) { send(:"#{transactable_type_name}").send(:<<, TransactableType.with_parameterized_name(transactable_type_name).first.transactables.build) if send(transactable_type_name).size.zero? }

          # used by cocoon gem to create nested forms
          define_method("build_#{transactable_type_name}") do
            tt = TransactableType.with_parameterized_name(transactable_type_name).first
