  ...
  class << self
    def decorate(configuration)
      ...
      Class.new(self) do
        ...
        inject_dynamic_fields(configuration, whitelisted: [:state_event, :name, :description, :capacity, :confirm_reservations, :location_id, :draft, :enabled, :deposit_amount, :quantity, :currency, :activated_at, :rank, :transactable_type_id, :transactable_type, :insurance_value, :rental_shipping_type, :dimensions_template_id, :shipping_profile_id, :tag_list, :minimum_booking_minutes, :seek_collaborators, :photo_ids, :category_ids, :attachment_ids, :waiver_agreement_template_ids, :topic_ids, :group_ids])
      end
    end
  end
