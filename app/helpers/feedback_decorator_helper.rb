module FeedbackDecoratorHelper
  def review_title
    if reservation?
      feedback_object.listing.name
    else
      feedback_object.product.name
    end
  end

  def order_image
    if reservation? && feedback_object.listing.has_photos?
      h.image_tag feedback_object.listing.photos.last.image_url(:medium)
    elsif line_item? && feedback_object.product.variant_images.present?
