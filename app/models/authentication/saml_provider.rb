class Authentication::SamlProvider
  ...
  def self.setup_proc
    lambda do |env|
      env['omniauth.strategy'].options[:issuer]                                      = "CN=sfederation.intel.com"
      env['omniauth.strategy'].options[:idp_sso_target_url]                          = "https://sfederation.intel.com/federation/IDZ_Devmesh.asp"
      env['omniauth.strategy'].options[:idp_cert_fingerprint_validator]              = lambda { |fingerprint| fingerprint }
