class User::PaymentTokenVerifier
  ...
  #
  # Returns String
  def generate(expires_at = 1.hours.from_now)
    payment_token = @user.generate_payment_token
    verifier.generate([@user.id, payment_token, @params, expires_at.to_i].join('|'))
  end

  def valid?(token)
    user_id, payment_token, params, expires_at = verifier.verify(token).split('|')
    user_id.to_i == @user.id && @user.verify_payment_token(payment_token) && Time.at(expires_at.to_i) > Time.now
  rescue ActiveSupport::MessageVerifier::InvalidSignature
    # If the signature is invalid, then we log the error and match no user.
    Rails.logger.warn("Invalid payment token: #{token} for user #{@user.id}")
    false
  end
  ...
  def self.find_token(token)
    ...
    user_id, payment_token, params, expires_at = extract_data_from_token(token)
    ...
    return [user, params] if user && new(user, params).valid?(token)
  ...
  def self.extract_data_from_token(token)
    ...
    user_id, payment_token, params, expires_at = Base64.decode64(data).split('|')
    [user_id.to_i, payment_token, JSON.parse(params), Time.at(expires_at.to_i)]
