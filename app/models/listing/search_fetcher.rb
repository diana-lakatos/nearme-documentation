  ...
  def listings
    @listings ||=
      begin
        ...
        filtered_listings = Transactable.searchable.where(location_id: _locations.map(&:id))
        filtered_listings = filtered_listings.filtered_by_listing_types_ids(@filters[:listing_types_ids]) if @filters[:listing_types_ids]
        filtered_listings = filtered_listings.filtered_by_price_types(@filters[:listing_pricing] & Transactable::PRICE_TYPES.map(&:to_s)) if @filters[:listing_pricing]
