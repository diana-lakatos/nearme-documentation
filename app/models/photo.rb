class Photo < ActiveRecord::Base
  ...
  def user_added_photos_to_project_event
    if owner_type == "Project" && owner.present?
      ...
      ActivityFeedService.create_event(event, self.owner, [self.owner.creator], self.owner) unless ActivityFeedEvent.where(followed: owner, event_source: owner, event: event, created_at: Time.now-1.minute..Time.now).count > 0
    end
  end
