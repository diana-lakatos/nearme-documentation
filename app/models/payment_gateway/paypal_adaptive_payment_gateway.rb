class PaymentGateway::PaypalAdaptivePaymentGateway < PaymentGateway
  ...
  def process_payout(merchant_account, amount, _reference)
    @pay = payout_gateway.build_pay(actionType: 'PAY',
    ...
    if @pay_response.success?
      if @pay_response.paymentExecStatus == 'COMPLETED'
        ...
        fail "Unknown payment exec status: #{@pay_response.paymentExecStatus}"
      end
    else
