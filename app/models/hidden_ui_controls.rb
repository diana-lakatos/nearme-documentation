class HiddenUiControls
  ...
  def self.to_obj(key)
    OpenStruct.new(name: key, tab?: is_key_tab?(key), hidden?: is_key_hidden?(key), visible?: is_key_visible?(key),
                   display_name: key_display_name(key), display_type: key_display_type(key))
  end
