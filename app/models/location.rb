class Location < ActiveRecord::Base
  ...
  scope :filtered_by_listing_type_ids,  lambda { |listing_types_ids| joins(:listings).where('listings.listing_type_id IN (?)', listing_types_ids) }
  scope :filtered_by_location_type_ids,  lambda { |location_types_ids| where('locations.location_type_id IN (?)', location_types_ids) }
