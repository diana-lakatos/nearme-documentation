class OldReservation < ActiveRecord::Base
  ...
  scope :not_migrated, -> { where(order_id: nil) }
  scope :migrated, -> { where(order_id: nil) }
  belongs_to :reservation, -> { with_deleted }, foreign_key: "order_id"
  ...
  has_many :periods, :class_name => "ReservationPeriod", :inverse_of => :reservation, :dependent => :destroy, foreign_key: 'old_reservation_id'
  has_many :shipments, dependent: :destroy, foreign_key: 'reservation_id'
  ...
  def mark_as_archived!
    if archived_at.nil?
      ...
      unless Rails.env.test?
        begin
          listing.__elasticsearch__.update_document_attributes(completed_reservations: listing.orders.reservations.reviewable.count)
  ...
  def action_hourly_booking?
    transactable_pricing.hour_booking?
  end

  def action_daily_booking?
    transactable_pricing.day_booking?
  end
  ...
  def schedule_expiry
    ...
    self.update_column(:expires_at, Time.current + hours_to_expiration)
    ReservationExpiryJob.perform_later(self.reload.expires_at, self.id) if hours_to_expiration > 0
  end
