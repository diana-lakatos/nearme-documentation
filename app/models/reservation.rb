class Reservation < ActiveRecord::Base
  ...
  scope :archived, lambda {
    joins(:periods).where('reservation_periods.date < ? OR reservations.state IN (?)', Date.today, ['rejected', 'expired', 'cancelled_by_host', 'cancelled_by_guest']).uniq
  }
  ...
  def archived?
    rejected? || cancelled? || (periods.all? {|p| p.date < Date.today} || expired?)
  end
