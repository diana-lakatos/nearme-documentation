class @InstanceAdmin.SearchServiceTypesController
  ...
  bindEvents: =>
    ...
    @container.find('.service_type_custom_attributes').click (event) ->
      event.preventDefault()
      clicker = $(event.target)
      $(clicker.attr('data-service-type-custom-attributes')).toggleClass('hidden')
  ...
  updateSearchable: (clicker) =>
    ...
    $.ajax
      ...
      url: checkbox.attr('data-url')
      dataType: 'JSON'
      data: { transactable_type: {searchable: checkbox.prop('checked')}}
      success: (res) =>
        spinner.addClass('hidden')
        checkbox.removeClass('hidden')